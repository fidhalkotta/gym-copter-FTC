...
class Hover3DV20(_Hover, _ThreeD):
    ...
    def _get_reward(self, status, state, d, x, y):
        ...
        c = - (w[0] * (x_r ** 2)) - (w[1] * (y_r ** 2)) - (w[2] * (z_r ** 2)) - (w[3] * (phi_r ** 2)) - (
                    w[4] * (theta_r ** 2))
...
class HoverVisual(Hover3DV20):
    RES = 16

    def __init__(self, vs=VisionSensor(res=RES)):
        Hover3DV20.__init__(self)
    ...
    def step(self, action):
        result = Hover3DV20.step(self, action)
