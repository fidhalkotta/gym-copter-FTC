...
class Hover3DV9(_Hover, _ThreeD):
    ...
    def _get_reward(self, status, state, d, x, y):
        ...
        phi_reward = self.gaussian_transform(angle_amplitude, angle_sigma, state[6])
        theta_reward = self.gaussian_transform(angle_amplitude, angle_sigma, state[8])
...
class HoverVisual(Hover3DV9):
    RES = 16

    def __init__(self, vs=VisionSensor(res=RES)):
        Hover3DV9.__init__(self)
    ...
    def step(self, action):
        result = Hover3DV9.step(self, action)
