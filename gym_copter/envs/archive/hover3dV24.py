...
class Hover3DV24(_Hover, _ThreeD):
    ...
    def _get_reward(self, status, state, d, x, y):
        ...
        phi_reward = gaussian_transform(angle_amplitude, angle_sigma, angularVelocity[0])
        theta_reward = gaussian_transform(angle_amplitude, angle_sigma, angularVelocity[1])
        psi_reward = gaussian_transform(angle_amplitude, angle_sigma, angularVelocity[2])

        reward = (0.1 * x_reward) + (0.1 * y_reward) + (0.5 * z_reward) \
                 + (0.1 * phi_reward) + (0.1 * theta_reward) + (0.1 * psi_reward)
...
class HoverVisual(Hover3DV24):
    RES = 16

    def __init__(self, vs=VisionSensor(res=RES)):
        Hover3DV24.__init__(self)
    ...
    def step(self, action):
        result = Hover3DV24.step(self, action)
