...
class Hover3DV26(_Hover, _ThreeD):
    ...
    def _get_reward(self, status, state, d, x, y):
        ...
        p = gaussian_transform(angle_amplitude, angle_sigma, angularVelocity[0])
        q = gaussian_transform(angle_amplitude, angle_sigma, angularVelocity[1])
        r = gaussian_transform(angle_amplitude, angle_sigma, angularVelocity[2])
...
class HoverVisual(Hover3DV26):
    RES = 16

    def __init__(self, vs=VisionSensor(res=RES)):
        Hover3DV26.__init__(self)
    ...
    def step(self, action):
        result = Hover3DV26.step(self, action)
