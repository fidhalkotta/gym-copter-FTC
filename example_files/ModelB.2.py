...
class ModelB_V3_example(_Hover, _ThreeD):
    def __init__(self, obs_size=12,
                 weights=None, enable_passive_faults=True,
    ...
    def set_fault_state(self, fault_magnitude):
        self.fault_map = fault_magnitude

    def reset(self):

        r = _Hover._reset(self)

        if self.enable_passive_faults:
            self.fault_map = random.choice(self.fault_cases)
...
class HoverVisual(ModelB_V3_example):
    RES = 16

    def __init__(self, vs=VisionSensor(res=RES)):
        ModelB_V3_example.__init__(self)
    ...
    def step(self, action):
        result = ModelB_V3_example.step(self, action)
